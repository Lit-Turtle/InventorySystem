...
def delete_inventory(invent: inventories):
    clear_screen()
    invent_name = input("Inventory Name: ")
    if invent_name in inventories:
        del inventories[invent_name]

#Finds category in inventory and returns whether inventory exists
def find_category(invent: inventories, invent_name, cate_name):
...
def get_category(invent: inventories, invent_name, cate_name):
...
def create_category(invent: inventories, invent_name):
    clear_screen()
...
def delete_category(invent: inventories, invent_name):
    clear_screen()
...
def delete_index(invent:inventories, invent_name):
    clear_screen()
...
def create_product(invent: inventories, invent_name, cate_name, id=None):
    clear_screen()
...
def delete_product(invent: inventories, invent_name, cate_name, pro_name_id):
...
def get_product(invent_name, cate_name, pro_name_id):
    return get_category(inventories, curr_cate[teste_met()].get_products(), cate_name).get_product(pro_name_id)

#Category level purchase
def purchased(inven: inventories, invent_name, cate_name):
    user_input = ''
    while(user_input != "0"):
        while(get_category(inventories, invent_name, cate_name).get_product(user_input) == "Not Found" or user_input == 0): 
        ...
        while invalid:
            if bool(re.search(r"\d", amount)):
                if get_category(inventories, invent_name, cate_name).get_product(user_input).get_supply() >= int(amount):
                    invalid = False
                    break
            amount = input("Invalid Amount. Try Again: ")
        get_category(inventories, invent_name, cate_name).purchased(user_input, int(amount))
        user_input = input("Enter Product Name or ID. Enter 0 when complete: ")

#Checks if input by user is a number. I dont think this is used!
def valid_num_input(supply):
    num = input("Enter Amount Sold: ")
    invalid = True

    while invalid:
        if bool(re.search(r"\d", num)):
            if supply >= int(num):
                invalid = False
                break
...
def invent_purchase(invent: inventories, invent_name):
    clear_screen()
    user_input = ''
    while(user_input != "0"):
...
def create_report(invent: inventories, invent_name):
    chart = ["Line 1", "Line 2"]
...
def create_account():
    ...
    while True: 
        ...
        #else if checks if account already exists by trying to login
        else:
...
create_account()
user = login()
inventories["Choco"].create_category("Food")

category = inventories["Choco"].get_category("Food")
if user.get("Invent2") and "Apple" in user["Invent2"]:
    category.create_product("Apple", 100, 1.00, 0.25, "Food", user["inventory"]["Invent2"]["Food"]["Apple"][2])     
else:
...
if user:
    ...
    inventories["Invent2"].create_category("Food")
    save_all(user, inventories)
    category = inventories["Invent2"].get_category("Food")
    save_all(user, inventories)
    if user.get("Invent2") and "Apple" in user["Invent2"]:
        category.create_product("Apple", 100, 1.00, 0.25, "Food", user["inventory"]["Invent2"]["Food"]["Apple"][2])     
    else:
...
#========User Menus=============
def menu_one(invent: inventories, session): #Inventories
    clear_screen()
    number_name_pair = {}
    count = 1
    for x,y in inventories.items():
    ...
    while (bool(re.search(r"\D", user_choice))):
    ...
    match(user_choice):
        ...
        case _ if user_choice == logout_index:
            session.logged_out()
            session.inventories = {} #
            menu_one(inventories, session)
            return
        case _ if user_choice == create_acc_index:
            user = create_account()
            session.set_user(user)
            save_all(session.get_user(), inventories)
            menu_one(inventories, session)
            return
        ...
        case _ if user_choice in number_name_pair:
            menu_two(inventories, number_name_pair[user_choice])
            return
        case _:
...
def menu_two(invent: inventories, invent_name): #Categories
    clear_screen()
    count = 1
    ...
    match(user_choice):
        case _ if user_choice == create_index:
            create_category(inventories, invent_name)
            if session.signed_in:
                save_all(session.get_user(), inventories)
            menu_two(inventories, invent_name)
            return
        case _ if user_choice == delete_index:
            if curr_cate:
                delete_category(inventories, invent_name)
                if session.signed_in:
                    save_all(session.get_user(), inventories)
            else:
        ...
        case _ if user_choice == sales_index:
            invent_purchase(inventories, invent_name)
            if session.signed_in:
                save_all(session.get_user(), inventories)
            menu_two(inventories, invent_name)
            return    
        ...
        case _ if user_choice == report_index:
            clear_screen()
            create_report(inventories, invent_name)
...
def menu_three(invent_name, products, cate_name): #Products
    clear_screen()
    products = get_category(inventories, invent_name, cate_name).get_products()
    count = 1
    for pro in products:
    ...
    match(user_choice):
        case _ if user_choice == create_index:
            create_product(inventories, invent_name, cate_name)
            if session.signed_in:
                save_all(session.get_user(), inventories)
        ...
        case _ if user_choice == delete_index:
            ...
            if products:
                ...
                delete_product(inventories, invent_name, cate_name, pro_name_id)
                if session.signed_in:
                    save_all(session.get_user(), inventories)
            else:
